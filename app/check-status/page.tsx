...
import { Search, FileText, Calendar, Clock, CheckCircle, AlertCircle, XCircle, Eye } from "lucide-react"

type ProjectStatus = "quote_requested" | "quote_sent" | "contract_signed" | "planning" | "pre_production" | "production" | "post_production" | "review" | "delivered" | "completed"
...
export default function CheckStatusPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || "Project not found. Please check your reference number and try again.")
      }
    } catch (error) {
  ...
  const getStageIcon = (stageId: ProjectStatus, isCompleted: boolean, isCurrent: boolean) => {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              Check Project <span className="bg-gradient-to-r from-[#BE55FF] to-[#70BFFF] bg-clip-text text-transparent">Status</span>
            </h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Enter your project reference number to track the progress of your project with Youu Media.
            </p>
          </div>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border border-gray-200 shadow-xl mb-8">
            <CardContent className="p-8">
              <div className="max-w-md mx-auto">
                <Label htmlFor="reference" className="text-lg font-semibold text-gray-900 mb-2 block">
          ...
          {project && (
            <div className="space-y-8">
                ...
                <CardHeader className="relative z-10">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-gray-700 mb-4">Client: {project.clientName}</p>
                ...
                <CardContent className="relative z-10">
                      <div className="grid md:grid-cols-3 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span><strong>Started:</strong> {project.startDate ? new Date(project.startDate).toLocaleDateString() : 'Not started yet'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-[#70BFFF]" />
                          <span><strong>Expected:</strong> {project.expectedCompletion ? new Date(project.expectedCompletion).toLocaleDateString() : 'TBD'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-[#70BFFF]" />
                          <span><strong>Last Updated:</strong> {new Date(project.lastUpdated).toLocaleDateString()}</span>
                        </div>
                      </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white/80 backdrop-blur-sm border border-gray-200 shadow-xl">
                <CardHeader>
                  <CardTitle className="text-2xl font-bold text-gray-900">Project Timeline</CardTitle>
                  <p className="text-gray-600">Track your project's progress through each stage</p>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {projectStages.map((stage, index) => {
                      const currentStageIndex = getCurrentStageIndex(project.currentStatus)
                      const isCompleted = index < currentStageIndex
                      const isCurrent = index === currentStageIndex
                      const isPending = index > currentStageIndex

                      return (
                        <div key={stage.id} className="flex items-start gap-4 p-4 rounded-lg bg-gray-50/50">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                              <h3 className={`font-semibold ${
                                ...
                                isCurrent ? 'text-[#70BFFF]' : 
                                'text-gray-500'
                              }`}>
                                {stage.name}
                              </h3>
                              {isCompleted && <CheckCircle className="h-4 w-4 text-green-600" />}
                              {isCurrent && <div className="w-2 h-2 bg-[#70BFFF] rounded-full animate-pulse" />}
                            </div>
                            <p className={`text-sm ${
              ...
              <Card className="bg-gradient-to-r from-[#70BFFF]/10 to-[#BE55FF]/10 border border-[#70BFFF]/20">
                <CardContent className="p-6">
                  ...
                  <p className="text-gray-700 mb-4">
                    If you have any questions about your project status or need to make changes, please don't hesitate to contact us.
                  </p>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div>
                      <p className="font-medium text-gray-900">Email</p>
                      <a href="mailto:contact@youumedia.com" className="text-[#70BFFF] hover:text-[#BE55FF] transition-colors">
                        contact@youumedia.com
                      </a>
                    </div>
                    <div>
                      <p className="font-medium text-gray-900">Phone</p>
                      <a href="tel:+447958189989" className="text-[#70BFFF] hover:text-[#BE55FF] transition-colors">
                        +447958189989
                      </a>
                    </div>
                  </div>
