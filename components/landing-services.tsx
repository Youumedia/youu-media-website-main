      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-4 md:gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="group relative bg-white p-4 md:p-8 rounded-3xl shadow-lg hover:shadow-2xl transition-all border border-gray-200 hover:border-transparent"
            >
              {/* Animated gradient background on hover */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#70BFFF]/10 via-[#BE55FF]/10 to-[#70BFFF]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl" />

              {/* Decorative corner element */}
              <div className="absolute top-0 right-0 w-16 h-16 md:w-24 md:h-24 bg-gradient-to-br from-[#70BFFF]/20 to-[#BE55FF]/20 rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity overflow-hidden rounded-tr-3xl" />
